...
def sig_naf_io_func(temp, t_naf, RH, p_Pt, p, method, model):
    ...
    if method == 'lam':
        ...
        lamb = 0.3 + 10.8*(RH/100) - 16*(RH/100)**2 + 14.1*(RH/100)**3
        sig_naf_io = 100*(0.005139*lamb - 0.00326)*np.exp(1268*(1/303 - 1/temp))
...
def rxn_areas_cs(w_Pt, t_cl, eps_gas, t_naf, r_c, r_Pt, rho_Pt, theta):
    ...
    geom_out['SApv_pt'] = (1 - eps_gas)*SA_Pt_agg / V_agg # Pt SA per V
    geom_out['SApv_dl'] = (1 - eps_gas)*SA_dl_agg / V_agg # Double layer SA per V
    geom_out['eps_n_cl'] = (1 - eps_gas)*V_naf_agg / V_agg # Nafion V fraction
...
""" Caclulate the reaction areas from Pt-loading and radii information """
# The following reaction areas function is defined for the flooded-agg micro-
# structure. In the function name, "fa" is used to stand for "flooded-agg."
"-----------------------------------------------------------------------------"
def rxn_areas_fa(w_Pt, t_cl, eps_g_cl, r_agg, t_agg, p_c, r_c, r_Pt, rho_Pt):
    # Units for inputs are:
    # w_Pt [mg/cm^2], t_cl [m], eps_g_cl [-], r_agg [m], t_agg [m], p_c [%],
    # r_c [m], r_Pt [m], and rho_Pt [kg/m^3]
    
    "Find the mass of Pt per agglomerate"
    w_Pt = w_Pt *0.01 # convert [mg/cm^2] --> [kg/m^2]
    V_agg = 4/3*np.pi*(r_agg + t_agg)**3
    V_agg_in = 4/3*np.pi*r_agg**3
    w_Pt_agg = w_Pt / t_cl / (1 - eps_g_cl) *V_agg
    
    "Distribute Pt mass to half sphere bulbs"
    V_c_agg = 4/3*np.pi*r_c**3 *(p_c/100) *(r_agg/r_c)**3
    V_Pt_agg = w_Pt_agg / rho_Pt
    V_Pt_bulb = 2/3*np.pi*r_Pt**3 # volume of Pt 1/2 sphere sitting on C surf
    N_bulbs = V_Pt_agg / V_Pt_bulb
    
    "Using r_Pt and assuming semi-spheres, find Pt surface area"
    SA_c_agg = 4*np.pi*r_c**2 *(p_c/100) *(r_agg/r_c)**3
    SA_Pt_agg = N_bulbs*2*np.pi*r_Pt**2
    SA_dl_agg = SA_c_agg - N_bulbs*np.pi*r_Pt**2 + SA_Pt_agg
    SA_naf_agg = 4*np.pi*(r_agg + t_agg)**2
    V_naf_agg = V_agg - V_c_agg - V_Pt_agg
    
    "Use SA and V factors to find geometric parameters to return"
    geom_out ={}
    geom_out['SApv_naf'] = (1 - eps_g_cl)*SA_naf_agg / V_agg
    geom_out['SApv_pt'] = (1 - eps_g_cl)*SA_Pt_agg / V_agg
    geom_out['SApv_dl'] = (1 - eps_g_cl)*SA_dl_agg / V_agg
    geom_out['eps_n_cl'] = (1 - eps_g_cl)*V_naf_agg / V_agg 
    geom_out['p_pt_agg'] = V_Pt_agg / V_agg_in *100 # % V of Pt in inner agglomerate
    geom_out['V_naf_agg'] = V_naf_agg
  
    # Units for outputs are:
    # SApv_* [1/m], eps_n_cl [-], p_pt_agg [%], V_naf_agg [m^3]
    return geom_out
...
from pemfc_transport_funcs import fickian_adf, radial_fdiff
from pemfc_property_funcs import sig_naf_io_func, D_eff_naf_func, rxn_areas

""" User Input Parameters """
"-----------------------------------------------------------------------------"
ctifile = 'pemfc_SD.cti'

" Initial electrochemical values "
i_OCV = 0                           # 0 [A/cm^2] -> or single if polar == 0
i_ext0 = np.linspace(0.001,0.1,3)   # external currents close to 0 [A/cm^2]
i_ext1 = np.linspace(0.101,0.2,3)   # external currents [A/cm^2]
i_ext2 = np.linspace(0.21,2.0,20)   # external currents further from 0 [A/cm^2]
...
R_naf = 70e-3        # resistance of Nafion membrane [Ohm*cm^2]
...
p_c = 95            # percentage of C sheres packed in agglomerate [%] - Gauss max = 74%
...
t_agg = 5e-9        # thickness of nafion shell around each agglomerate [m]
...
radial = 0          # turn on radial O2 plots for each agglomerate
...
i_find = 1.0        # current from polarization curve to use in i_val validation
ver = 1             # temporary debugging tool for radial diffusion
...
" Store phases in a common 'obj' dict: "
obj = {}
obj['carb_ca'] = carb_ca
obj['gas_ca'] = gas_ca
...
if area_calcs == 0: # Calcs for areas based on % Pt and flat circles
    ...
    SApv_naf = 3*(1 - eps_g_cl) / (r_agg +t_agg)

    # Double layer surface area per total volume [m^2 cathode / m^3 tot]
    SApv_dl = 3*(1 - eps_g_cl)*(p_c/100)*r_agg**3 / (r_agg +t_agg)**3 / r_c

    # Pt surface area per total volume [m^2 Pt / m^3 tot]
    SApv_pt = 3*(1 - eps_g_cl)*(p_c/100)*r_agg**3*(p_Pt/100) / (r_agg +t_agg)**3 / r_c

    # Volume fraction of nafion in CL [-]
    eps_n_cl = (1 - eps_g_cl)*((r_agg +t_agg)**3 - (p_c/100)*r_agg**3) / (r_agg +t_agg)**3

    # Volume fraction of nafion in each inner agglomerate [-]:
    eps_n_agg = 1 - (p_c/100)

    # Volume fraction of nafion in each agglomerate core and shell [-]:
    V_naf_agg = 4/3*np.pi*(r_agg +t_agg)**3 - 4/3*np.pi*(r_agg)**3 + 4/3*np.pi*(r_agg)**3*eps_n_agg
    Vf_naf_shl = (4/3*np.pi*(r_agg +t_agg)**3 - 4/3*np.pi*(r_agg)**3) / V_naf_agg
    ...
    geom_out = rxn_areas(w_Pt, t_cl, eps_g_cl, r_agg, t_agg, p_c, r_c, r_Pt, rho_Pt)
...
tau_agg = 1.05 #eps_n_agg**(-0.5)
tau_gdl = eps_g_gdl**(-0.5)
tau_cl = eps_g_cl**(-0.5)
tau_cl_n = eps_n_cl**(-0.5)

# Radius vectors for diffusion calculations [m]:
r_j = np.hstack([r_agg +t_agg/2, np.linspace(r_agg -r_agg/(2*Nr_cl), r_agg/(2*Nr_cl), Nr_cl)])
r_jph = np.hstack([r_agg, r_j[1:Nr_cl] -r_agg/(2*Nr_cl)])
t_shl = np.hstack([t_agg, np.tile(r_agg/Nr_cl, Nr_cl)])
dr = abs(np.diff(r_j))
...
gdl['eps/tau2'] = eps_g_gdl / tau_gdl**2
gdl['y'] = t_gdl
gdl['Ny'] = Ny_gdl
...
cl['eps/tau2'] = eps_g_cl / tau_cl**2
cl['eps/tau2_n'] = eps_n_agg / tau_agg**2 # inner agglomerate only
cl['eps/tau2_n2'] = eps_n_cl #/ tau_cl_n**2 # based on full cell Nafion vol frac
cl['y'] = t_cl
cl['Ny'] = Ny_cl
cl['Nr'] = Nr_cl
...
cl['D_eff_naf'] = D_eff_naf_func(naf_b_ca.T, t_agg, p_Pt, D_O2_method)
cl['sig_naf_io'] = sig_naf_io_func(naf_b_ca.T, t_agg, RH, p_Pt, cl, sig_method)
cl['1/r_j'] = 1 / r_j
cl['r_jph'] = r_jph
cl['1/t_shl'] = 1 / t_shl
cl['1/dr'] = 1 / dr
...
" Combine all dictionaries and load additional general parameters "
p = {}
p['gdl'] = gdl
p['gdl_cl'] = gdl_cl
p['cl'] = cl
...
def dsvdt_func(t, sv, objs, p, iSV):
    ...
    for i in range(cl['Ny'] -1):
        ...
        # Set the phases for absorption rxn:
        ...
        rho_dot_n = naf_s_ca.get_net_production_rates(naf_b_ca) *cl['SApv_naf']\
                  *cl['1/eps_n'] *naf_b_ca.molecular_weights #*cl['1/Vf_shl'][0]
        rho_dot_n = naf_s_ca.get_net_production_rates(naf_b_ca)*cl['r_agg']**2\
                  *naf_b_ca.molecular_weights #*cl['1/Vf_shl'][0]

        #print(cl['r_agg'],cl['r_jph'])
        ...
        # This node contains an O2 absoroption rxn with the gas phase as well
        # as a mass flux with the inner agglomerate.
        rho_k1 = sv[iSV['rho_shl_k'] +cl_ymv]
        rho_k2 = sv[iSV['rho_naf_k'] +cl_ymv]
        rho_flx_inr = radial_fdiff(rho_k1, rho_k2, cl, 0, ver)

        # Combine absorption and flux to get overall ODE:
        dsvdt[iSV['rho_shl_k'] +cl_ymv] = (o2_abs *rho_dot_n - rho_flx_inr) *cl['1/r_j'][0]**2*cl['1/t_shl'][0]
        ...
        for j in range(cl['Nr'] -1):
            ...
            rho_flx_inr = radial_fdiff(rho_k1, rho_k2, cl, j, ver)
            ...
            dsvdt[iMid] = (rho_flx_otr - rho_flx_inr)*cl['1/r_j'][j]**2 \
                *cl['1/t_shl'][j] + pt_rxn *rho_dot_n
        ...
        " Apply symmetric flux BC at inner-most agglomerate node "
        ...
        dsvdt[iLast] = (rho_flx_otr - rho_flx_inr)*cl['1/r_j'][-1]**2 \
            *cl['1/t_shl'][-1]  + pt_rxn *rho_dot_n
    ...
    """ Exit loop, set BC for CL and Electrolyte Boundary """
    # Temperature at last Y node:----------------------------------------------
    dsvdt[iSV['T_cl'] +cl_ymv] = 0

    # Gas phase species at last Y node:----------------------------------------
    flux_dwn = np.zeros(gas_ca.n_species)

    # Set the phases for absorption rxn:
    rho_gas_k = sv[iSV['rho_gas_k'] +cl_ymv]
    rho_naf_k = sv[iSV['rho_shl_k'] +cl_ymv]
    ...
    rho_dot_g = naf_s_ca.get_net_production_rates(gas_ca) *cl['SApv_naf']\
              *cl['1/eps_g'] *gas_ca.molecular_weights *gas_tog
    rho_dot_n = naf_s_ca.get_net_production_rates(naf_b_ca) *cl['SApv_naf']\
              *cl['1/eps_n'] *naf_b_ca.molecular_weights#*cl['r_agg']**2 #*cl['1/Vf_shl'][0]

    rho_dot_n = naf_s_ca.get_net_production_rates(naf_b_ca)*cl['1/eps_n']\
                *naf_b_ca.molecular_weights*cl['r_agg']**2 #*cl['1/Vf_shl'][0]

    # Include rxn and flux in ODE term:
    dsvdt[iSV['rho_gas_k'] +cl_ymv] = (flux_up - flux_dwn)*cl['1/eps_g']*cl['1/dy']\
                                    + o2_abs *rho_dot_g

    # Double layer potential at last Y node:-----------------------------------
    " Start by evaluating single-node nafion shell "
    # This node contains an O2 absoroption rxn with the gas phase as well
    # as a mass flux with the inner agglomerate.
    rho_k1 = sv[iSV['rho_shl_k'] +cl_ymv]
...
if post_only == 0:
    ...
    cl['i_ext'] = i_OCV*(100**2)

    sol = solve_ivp(lambda t,sv: dsvdt_func(t, sv, obj, p, iSV),
          [0, t_sim], SV_0, method=method, atol=atol, rtol=rtol, max_step=max_t)

    # Store solution and update initial values:
    SV_0 = sol.y[:,-1]
    sv_save[:,0] = np.append(i_OCV, sol.y[:,-1])
    dphi_ss[0] = sol.y[iPhi_f, -1] - dphi_eq_an - cl['i_ext']*pem['R_naf'] / 100**2\
               - cl['i_ext']*(0.5*cl['dy'] / cl['sig_naf_io'])
    ...
    if all([i_OCV == 0, polar == 1]):
        for i in range(len(i_ext)):
            ...
            dphi_ss[i+1] = sol.y[iPhi_f,-1] - dphi_eq_an - cl['i_ext']*pem['R_naf'] / 100**2\
                         - cl['i_ext']*(0.5*cl['dy'] / cl['sig_naf_io'])

            print('t_f:', sol.t[-1], 'i_ext:', round(cl['i_ext']*1e-4,3),
...
if polar == 1:
    ...
    try:
        plt.errorbar(x, y, yerr=yerr, fmt='.', color=color, capsize=3)
        plt.ylim([0.35, 1.0])
        plt.xlim([0, 2.1])
    except:
        None
...
if i_val == 1:
    ...
    i_4F = i_all[i_ind]*100**2 / (4*ct.faraday)
    print('\nO2_i_4F:', i_4F)
    ...
    print('i_ext:', i_all[i_ind], 'O2_BC_flux:', O2_BC_flux[0],

...
def radial_fdiff(rho_k1, rho_k2, p, node):
    ...
    # 'r_jph': radius at bounday between nodes (j+1/2) [1/m]
    # '1/dr': invserse of distance between nodes [1/m]
    # 'p_eff_SAnaf': effective Nafion SA based on Pt and max angle [-]
    
    drho_dt = p['D_eff_naf'] *p['p_eff_SAnaf']\
            *(p['r_jph'][node]**2 *(rho_k2 - rho_k1) *p['1/dr'][node])
            
    # Output returns change in density vector [kg/s]
    return drho_dt
"-----------------------------------------------------------------------------"

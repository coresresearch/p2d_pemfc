...
def dsvdt_func(t, sv, objs, p, ptrs):
    ...
    for i in range(p['Ny']-1):
        ...
        dsvdt[iFirst] = p['1/eps_naf']*sdot_i_naf*p['SA_pv_pt']*p['1/Vf_shl'][0] \
                      + flux_in *p['1/r_j'][0]**2 *p['1/t_shl'][0]
        ...
        for j in range(1, p['Nr']-1):
            ...
            dsvdt[iSV['rho_n1']+j*naf_mv+sv_mv] = (flux_in - flux_out)\
                                                *p['1/r_j'][j]**2 *p['1/t_shl'][j]
            
            rho_k1 = rho_k2                                          
            flux_out = flux_in

        "Outermost node has gas-interface reaction BC and diffusion out"
        iLast = iSV['rho_n1']+(p['Nr']-1)*naf_mv+sv_mv
        dsvdt[iLast] = p['1/eps_naf']*sdot_o_naf*p['SA_pv_naf']*p['1/Vf_shl'][-1] \
                     - flux_out *p['1/r_j'][-1]**2 *p['1/t_shl'][-1]

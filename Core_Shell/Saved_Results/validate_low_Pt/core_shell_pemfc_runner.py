...
i_ext1 = np.linspace(0.101,0.2,10)  # external currents [A/cm^2]
i_ext2 = np.linspace(0.21,2.13,20)  # external currents further from 0 [A/cm^2]
phi_ca_init = 0.3                   # initial cathode potential [V]
...
sig_method = 'lam'   # 'lam', 'bulk', or 'mix' for conductivity method
D_O2_method = 'lam'  # 'lam', 'bulk', or 'mix' for D_O2 method
R_naf = 120e-3       # resistance of Nafion membrane [Ohm*cm^2]
...
w_Pt = 0.025        # loading of Pt on carbon [mg/cm^2]
rho_Pt = 21.45e3    # density of Pt for use in area property calcs [kg/m^3]
r_c = 45e-9         # radius of single carbon particle [m]
...
post_only = 1       # turn on to only run post-processing plots
...
" Saving options "
folder_name = 'validate_low_Pt'  # folder name for saving all files/outputs

...
"-----------------------------------------------------------------------------"
def sig_naf_io_func(temp, t_naf, RH, p_Pt, method):
    ...
    sig_naf_io_mix = 1 / (p_Pt/sig_naf_io_lam + (1-p_Pt)/sig_naf_io_bulk)
    
    " Set conductivity depending on method "
    # Based on the method, return the appropriate conductivity.
    if method == 'lam':
        sig_naf_io = sig_naf_io_lam
    elif method == 'bulk':
        sig_naf_io = sig_naf_io_bulk 
    elif method == 'mix':
        sig_naf_io = sig_naf_io_mix
...
def D_eff_naf_func(temp, t_naf, p_Pt, method):
    ...
    # lamellae and that through bulk-like material is performed with respect to
    # the amount of Pt and C areas respectively.
    D_eff_naf_mix = 1 / (p_Pt/D_eff_naf_lam + (1-p_Pt)/D_eff_naf_bulk)
...
"-----------------------------------------------------------------------------"
def rxn_areas(w_Pt, t_cl, eps_gas, t_naf, r_c, r_Pt, rho_Pt):
    ...
    geom_out ={}
    geom_out['SA_pv_naf'] = (1 - eps_gas)*SA_naf_int / V_agg # Nafion SA per V

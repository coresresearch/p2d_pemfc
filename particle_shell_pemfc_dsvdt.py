...
def dsvdt_func(t, sv, objs, p, ptrs):
    ...
    sv_nxt = int((len(p['SV_0'])-(1+gas_ca.n_species)*p['Ny_gdl'])*p['1/Ny'])

    gdl_mv = 0 # Start with the first GDL node (y: gas channel -> CL)
    gdl_nxt = int(gas_ca.n_species) + 1
...
dphi_ss[0] = sol.y[iPhi_f, -1] - dphi_eq_an
...
if i_OCV == 0:
    for i in range(len(i_ext)):
        ...
        dphi_ss[i+1] = sol.y[iPhi_f,-1] - dphi_eq_an
